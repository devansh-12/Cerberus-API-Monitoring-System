...
test.describe('Auth API Integration Tests', () => {
  test('POST /api/auth/login - validation rejects missing password', async ({ request }) => {
  ...
  test('GET /api/auth/profile - rejects without auth token', async ({ request }) => {
...
test.describe('Client API Integration Tests', () => {
  test('GET /api/client/admin/clients - rejects without auth', async ({ request }) => {
...
test.describe('API Contract Tests', () => {
  test('all responses have consistent JSON structure', async ({ request }) => {
    ...
    for (const { method, path, data } of endpoints) {
      const response = await (request as any)[method](path, { data });
