...
// Service definitions
const SERVICES = [
  { name: 'user-service', endpoints: ['/api/users', '/api/users/:id', '/api/users/profile', '/api/users/settings'] },
  { name: 'payment-service', endpoints: ['/api/payments', '/api/payments/:id', '/api/payments/process', '/api/payments/refund'] },
  { name: 'order-service', endpoints: ['/api/orders', '/api/orders/:id', '/api/orders/status', '/api/orders/history'] },
  { name: 'notification-service', endpoints: ['/api/notifications', '/api/notifications/send', '/api/notifications/preferences'] },
  { name: 'analytics-service', endpoints: ['/api/analytics/dashboard', '/api/analytics/reports', '/api/analytics/export'] },
];
...
async function seedDatabase() {
  ...
  try {
    ...
    const clientTemplates = [
      { name: 'Acme Corporation', slug: 'acme-corp', email: 'admin@acme.io', website: 'https://acme.io' },
      { name: 'TechStart Inc', slug: 'techstart', email: 'admin@techstart.io', website: 'https://techstart.io' },
      { name: 'DataFlow Systems', slug: 'dataflow', email: 'admin@dataflow.io', website: 'https://dataflow.io' },
      { name: 'CloudNine Solutions', slug: 'cloudnine', email: 'admin@cloudnine.io', website: 'https://cloudnine.io' },
      { name: 'NexGen Analytics', slug: 'nexgen', email: 'admin@nexgen.io', website: 'https://nexgen.io' },
    ];
    ...
    for (let i = 0; i < config.numClients; i++) {
      ...
      for (const env of environments) {
        const apiKeyRecord = await ApiKey.create({
          ...
          permissions: {
            ...
            allowedServices: env === 'production' ? [] : ['user-service', 'order-service'],
          },
          security: {
      ...
      console.log(`   Admin: ${adminUser.username} (${adminUser.email}) - Password: ${clientPassword}`);
      console.log(`   Viewer: ${viewerUser.username} (${viewerUser.email}) - Password: ${viewerPassword}`);
      console.log(`   API Keys: ${clientApiKeys.length} (${environments.join(', ')})`);
    }
    ...
    for (let clientIndex = 0; clientIndex < clients.length; clientIndex++) {
      ...
      const clientKeys = apiKeys.filter(k => k.ClientId.toString() === client._id.toString());
      ...
      console.log(`✅ Generated ${config.numHitsPerClient} hits for ${client.name}`);
    }
    ...
    for (let i = 0; i < allApiHits.length; i += BATCH_SIZE) {
      ...
      console.log(`   Inserted ${Math.min(i + BATCH_SIZE, allApiHits.length)}/${allApiHits.length} hits`);
    }
    ...
    for (let i = 0; i < metricsArray.length; i += BATCH_SIZE) {
      ...
      for (const metric of batch) {
        const avgLatency = metric.latencies.reduce((sum, val) => sum + val, 0) / metric.latencies.length;
      ...
      console.log(`   Processed ${Math.min(i + BATCH_SIZE, metricsArray.length)}/${metricsArray.length} metric buckets`);
    }
    console.log(`✅ PostgreSQL: ${metricsArray.length} metric buckets inserted`);
    ...
    console.log('\n📈 Summary:');
    console.log(`   • Users: ${1 + clientAdmins.length + clientViewers.length} (1 super admin + ${clientAdmins.length} client admins + ${clientViewers.length} viewers)`);
    ...
    console.log('\n🔐 Credentials:');
    console.log(`   Super Admin: superadmin@cerberus.io / ${superAdminPassword}`);
    for (let i = 0; i < clients.length; i++) {
      console.log(`\n   ${clients[i].name}:`);
      console.log(`     Admin: ${clientAdmins[i].email} / [generated password]`);
      console.log(`     Viewer: ${clientViewers[i].email} / [generated password]`);
    }

    console.log('\n📊 Data Distribution:');
    console.log(`   • Time range: Last 30 days`);
    console.log(`   • Services: ${SERVICES.map(s => s.name).join(', ')}`);
    console.log(`   • Status code distribution: ~70% success, ~20% client error, ~10% server error`);
