...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract message from Axios error response, fallback to generic
    ...
    <div className="bg-surface-background text-on-surface min-h-screen flex flex-col items-center justify-center p-6 antialiased">
      <div className="w-full max-w-[420px] flex flex-col items-center">
        ...
        <div className="w-full bg-surface-card border border-surface-border rounded-xl p-8 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-surface-border to-transparent opacity-50" />

          {/* Server error banner */}
          {error && (
            <div className="mb-5 flex items-start gap-2 p-3 rounded-lg bg-status-error/10 border border-status-error/30 text-status-error text-sm">
          ...
          <form className="flex flex-col space-y-5" onSubmit={handleLogin} noValidate>
            <div className="flex flex-col space-y-2">
              ...
              <input
                ...
                disabled={isLoading}
                className="bg-surface border border-surface-border rounded text-text-primary font-body-base text-body-base px-4 py-3 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all duration-200 disabled:opacity-60"
              />
            </div>

            <div className="flex flex-col space-y-2">
              <div className="flex justify-between items-center">
                <label className="font-body-sm text-body-sm text-text-secondary font-medium" htmlFor="password">
                  Password
                </label>
                <button
                  type="button"
            ...
            <div className="pt-4">
              <button
                className="w-full bg-primary-container hover:bg-primary text-on-primary-container font-body-base text-body-base font-semibold py-3 px-4 rounded transition-all duration-200 flex items-center justify-center space-x-2 disabled:opacity-60 disabled:cursor-not-allowed"
                type="submit"
                id="login-submit-btn"
                disabled={isLoading}
              >
