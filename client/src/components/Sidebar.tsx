...
const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  `flex items-center gap-3 px-3 py-2 rounded-DEFAULT transition-colors duration-200 ease-in-out ${
    isActive
      ? 'bg-slate-800 dark:bg-[#334155] text-white border-r-2 border-[#3B82F6]'
...
const Sidebar: React.FC = () => {
  ...
  return (
    <nav className="bg-slate-900 dark:bg-[#1E293B] font-['Space_Grotesk'] antialiased h-screen w-64 border-r fixed left-0 top-0 border-slate-800 dark:border-[#334155] flex flex-col py-4 z-50">
      ...
      {/* Footer links */}
      <div className="px-3 pt-4 border-t border-slate-800 dark:border-[#334155] mt-auto space-y-1">
        <button type="button" className="flex items-center gap-3 px-3 py-2 rounded-DEFAULT text-slate-400 hover:text-white hover:bg-slate-800/50 transition-colors duration-200 ease-in-out w-full">
